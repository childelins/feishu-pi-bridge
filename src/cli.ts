...
import {
  ...
  statSync, createReadStream, watch,
} from 'node:fs';
...
export async function status(): Promise<void> {
  ...
  console.log(`  Started:      ${new Date(resp.startedAt).toLocaleString()} (running ${formatDuration(uptimeMs)})`);
  console.log(`  Log:          ${config.paths.logFile}`);
  console.log(`  Active chats: ${resp.chats.length}`);
  for (const c of resp.chats) {
    const idleMs = Date.now() - c.lastActiveAt;
    console.log(`    - ${c.chatId} (rpc pid ${c.pid ?? 'n/a'}, last active ${formatDuration(idleMs)} ago)`);
  }
}
